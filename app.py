...
def github_callback():
    ...
    print("User Data from GitHub:", user_data)  # Add this line
...
def welcome():
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT username, email FROM user WHERE github_id = ?', (github_id,))
    user = c.fetchone()
    conn.close()

    return render_template('welcome.html', username=user[0], email=user[1])
